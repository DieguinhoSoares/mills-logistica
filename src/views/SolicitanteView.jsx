...
function RequestForm({ simClients, onSubmit, onClose, profile, initialData }) {
  ...
  const errStyle = { color:T.perigo, fontSize:10, fontFamily:FONT, marginTop:3 }
  const fieldErr = k => errors[k] ? <div style={errStyle}>⚠ Campo obrigatório</div> : null
          ...
          {form.clientName && form.selectedClient?.nInternos?.length > 0 && (
            <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginTop:6 }}>
              {form.selectedClient.nInternos
                .filter(n => !form.nInternos.includes(n))
                .slice(0,8)
                .map(n => (
                  <span key={n} onClick={()=>set('nInternos',[...form.nInternos, n])}
                    style={{ background:T.verdeLight, border:`1px solid ${T.verde}40`, borderRadius:20, padding:'2px 10px', fontSize:10, fontFamily:FONT, fontWeight:700, color:T.verde, cursor:'pointer' }}>
                    + {n}
                  </span>
                ))
              }
        ...
        <AnimatePresence>
          ...
          {needsNF && (
            <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }} exit={{ opacity:0, height:0 }}
              ...
              <ChipInput
                label="📄 Nota(s) Fiscal(is) de Retorno"
      ...
      <div style={{ maxWidth:860, margin:'0 auto', padding:'24px 12px' }}>
        <motion.div initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }}
          ...
          <button onClick={()=>setShowForm(true)}
            style={{ ...BS, background:T.laranja, color:'white', fontWeight:900, fontSize:14, padding:'13px 22px', borderRadius:T.rLg,
              boxShadow:'0 4px 16px rgba(243,112,33,.4)', whiteSpace:'nowrap', position:'relative', zIndex:1 }}>
            + Nova Solicitação
          </button>
        </motion.div>
        ...
        <h3 style={{ color:T.text, fontFamily:FONT, fontWeight:800, fontSize:16, margin:'0 0 12px' }}>Minhas Solicitações</h3>
              ...
              <motion.div key={r.id} layout initial={{ opacity:0, y:5 }} animate={{ opacity:1, y:0 }}
                ...
                {nfs.length > 0 && (
                  <div style={{ display:'flex', gap:5, flexWrap:'wrap', marginBottom:8 }}>
                    ...
                    {nfs.map(n => (
                      <span key={n} style={{ background:T.verdeLight, color:T.verde, borderRadius:20, padding:'1px 8px', fontSize:10, fontWeight:700, fontFamily:FONT }}>{n}</span>
                    ))}
                  </div>
                )}
