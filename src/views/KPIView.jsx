...
import { T, FONT, CARD_TYPES, MONTH_NAMES } from '../lib/constants'
    ...
    <div style={{ background:T.surface, borderRadius:T.rLg, border:`1px solid ${T.border}`, padding:'14px 16px', boxShadow:T.shadow }}>
      ...
      <div style={{ display:'flex', gap:14, alignItems:'center' }}>
        <svg width="82" height="82" viewBox="0 0 82 82" style={{ flexShrink:0 }}>
          ...
          <text x="41" y="46" textAnchor="middle" fontFamily="Nunito,sans-serif" fontWeight="900" fontSize="13" fill={T.text}>{total}</text>
        </svg>
...
export function KPIView({ cards, requests }) {
  const stats = useMemo(() => {
    ...
    const byType = Object.entries(CARD_TYPES).map(([k,v])=>({ label:v.short, value:cards.filter(c=>c.type===k).length, color:v.color }))
    const driverMap={}; cards.forEach(c=>{ const d=c.driver||'Sem motorista'; driverMap[d]=(driverMap[d]||0)+1 })
    const byDriver = Object.entries(driverMap).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([k,v])=>({label:k,value:v}))
    const stateMap={}; cards.forEach(c=>{ const s=c.destState||c.destination; if(s) stateMap[s]=(stateMap[s]||0)+1 })
    const byState = Object.entries(stateMap).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([k,v])=>({label:k,value:v}))
    const filialMap={}; cards.forEach(c=>{ const f=c.unit; if(f) filialMap[f]=(filialMap[f]||0)+1 })
    const byFilial = Object.entries(filialMap).sort((a,b)=>b[1]-a[1]).slice(0,5).map(([k,v])=>({label:k.replace(/ \(.*\)/,''),value:v}))
    const cardsComDatas = cards.filter(c=>c.startDate&&c.endDate)
    let tempoMedio = 0
    if (cardsComDatas.length) {
      const totalDias = cardsComDatas.reduce((s,c)=>s+Math.max(0,(new Date(c.endDate)-new Date(c.startDate))/86400000),0)
      tempoMedio = Math.round(totalDias/cardsComDatas.length*10)/10
    }
    const routeMap={}
    cards.filter(c=>c.origin&&c.destination).forEach(c=>{ const k=`${c.originCityName||c.origin} → ${c.destCityName||c.destination}`; routeMap[k]=(routeMap[k]||0)+1 })
  ...
  return (
    <div style={{ padding:'16px 20px', overflowY:'auto', height:'100%' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:14 }}>
        ...
        <KPICard title="Tempo Médio"        value={`${stats.tempoMedio}d`} icon="⏱" color={T.info}    bg={T.infoLight}     sub="Duração por serviço"/>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:14 }}>
