...
function ExportModal({ cards, onClose }) {
  ...
  const filtered = cards.filter(c => {
    ...
    setCell(wsR, `A${footRow+1}`, 'mills infraestrutura · Segurança para sonhar mais alto', footSt)
    ...
    const wsD = XLSX.utils.aoa_to_sheet([
      ['mills infraestrutura · Relatório Detalhado de Operações', ...Array(13).fill('')],
    ...
    const wsM = XLSX.utils.aoa_to_sheet([
      ['mills infraestrutura · Consolidado por Motorista', ...Array(6).fill('')],
          ...
          <div style={{ display:'flex', gap:6, marginTop:10, flexWrap:'wrap' }}>
            {[
              ...
              <button key={lbl} onClick={() => { setDateFrom(f); setDateTo(t); setDone(false) }}
                style={{ ...BS, background: dateFrom===f && dateTo===t ? T.laranja : T.surface,
                  color: dateFrom===f && dateTo===t ? 'white' : T.textSec,
                  border:`1px solid ${dateFrom===f && dateTo===t ? T.laranja : T.border}`,
