...
import { T, CARD_TYPES, CARD_SUBTYPES, URGENCY, BR_STATES, FILIAIS, MONTH_NAMES, WD_SHORT, BS, IS, LS, NB } from '../lib/constants'
...
function RequestReviewModal({ req, teamsWebhookUrl, onRespond, onClose, simClients }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(26,22,18,.55)', zIndex:1200, display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(4px)' }}
      ...
      <motion.div initial={{ scale:.95, opacity:0, y:10 }} animate={{ scale:1, opacity:1, y:0 }}
        ...
        {/* Estimativa de frete — calculada automaticamente pelo peso da máquina */}
        {(req.type === 'freteMillsInterno' || req.type === 'freteCliente') && (
...
/* ══ REQUESTS KANBAN ══════════════════════════════════════════════════════════ */
function RequestsKanban({ requests, teamsWebhookUrl, onRespond, simClients }) {
  ...
  return (
    <>
      {reviewing && <RequestReviewModal req={reviewing} teamsWebhookUrl={teamsWebhookUrl} onRespond={onRespond} simClients={simClients} onClose={()=>setReviewing(null)}/>}
        ...
        {cols.map(col => (
          <div key={col.key} style={{ background:col.bg, borderRadius:T.rLg, border:`1px solid ${col.color}30`, padding:14, display:'flex', flexDirection:'column', overflow:'hidden' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12, flexShrink:0 }}>
              ...
              <span style={{ color:T.text, fontFamily:'Barlow Condensed,IBM Plex Sans,sans-serif', fontWeight:700, fontSize:15, textTransform:'uppercase', letterSpacing:'0.04em' }}>{col.label}</span>
              <span style={{ marginLeft:'auto', background:col.color, color:'white', borderRadius:20, padding:'1px 8px', fontSize:11, fontWeight:800, fontFamily:'IBM Plex Sans,sans-serif' }}>{groups[col.key].length}</span>
            </div>
            <div style={{ overflowY:'auto', flex:1, display:'flex', flexDirection:'column', gap:10 }}>
              {groups[col.key].length === 0 && <div style={{ textAlign:'center', color:T.textMuted, fontFamily:'IBM Plex Sans,sans-serif', fontSize:12, paddingTop:20 }}>—</div>}
                ...
                return (
                  <motion.div key={r.id} layout whileHover={{ y:-1, boxShadow:T.shadowMd }}
                    ...
                    <div style={{ display:'flex', gap:6, marginBottom:8, flexWrap:'wrap' }}>
                      <span style={{ background:ct?.bg, border:`1px solid ${ct?.color}40`, borderRadius:20, padding:'2px 8px', color:ct?.color, fontSize:9, fontWeight:700, fontFamily:'IBM Plex Sans,sans-serif' }}>{ct?.icon} {ct?.short}</span>
                      <span style={{ background:ug?.bg, borderRadius:20, padding:'2px 8px', color:ug?.color, fontSize:9, fontWeight:700, fontFamily:'IBM Plex Sans,sans-serif' }}>{ug?.icon}</span>
                      {r.subtype && <span style={{ background:T.infoLight, borderRadius:20, padding:'2px 8px', color:T.info, fontSize:9, fontWeight:700, fontFamily:'IBM Plex Sans,sans-serif' }}>{getSubtypeLabel(r.type, r.subtype)}</span>}
                      <span style={{ marginLeft:'auto', fontSize:11 }}>{r.channel==='teams'?'🟦':r.channel==='whatsapp'?'💬':'📧'}</span>
                    </div>
                    <div style={{ color:T.text, fontWeight:700, fontSize:13, fontFamily:'IBM Plex Sans,sans-serif', marginBottom:4 }}>{r.requesterName||'—'}</div>
                    <div style={{ color:T.textSec, fontSize:11, fontFamily:'IBM Plex Sans,sans-serif', marginBottom:3 }}>{r.unit}</div>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      <span style={{ color:T.textMuted, fontSize:10, fontFamily:'IBM Plex Sans,sans-serif' }}>🔧 {r.machine||'—'}</span>
                      <span style={{ color:T.textMuted, fontSize:10, fontFamily:'IBM Plex Sans,sans-serif' }}>📅 {fmt(r.desiredDate)}</span>
                    </div>
                    {r.description && <div style={{ marginTop:6, color:T.textMuted, fontSize:10, fontFamily:'IBM Plex Sans,sans-serif', fontStyle:'italic' }}>"{r.description.slice(0,60)}{r.description.length>60?'...':''}"</div>}
                    {col.key==='pendente' && <div style={{ marginTop:8, textAlign:'right' }}>
                      <span style={{ background:T.laranjaLight, border:`1px solid ${T.laranja}40`, borderRadius:5, padding:'2px 8px', color:T.laranja, fontSize:10, fontWeight:700, fontFamily:'IBM Plex Sans,sans-serif' }}>Avaliar →</span>
                    </div>}
                    {r.responseNote && <div style={{ marginTop:6, padding:'5px 8px', background:col.key==='aceito'?T.verdeLight:T.perigoLight, borderRadius:T.rSm, color:col.key==='aceito'?T.verde:T.perigo, fontSize:10, fontFamily:'IBM Plex Sans,sans-serif' }}>{r.responseNote}</div>}
                  </motion.div>
                )
              })}
            </div>
      ...
      {activeTab==='requests'&&(
        <div style={{ flex:1, overflow:'hidden', padding:'14px 20px', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ flex:1, overflow:'hidden' }}>
            <RequestsKanban requests={requests} teamsWebhookUrl={config?.teamsWebhookUrl} onRespond={handleRespond} simClients={simClients}/>
          </div>
        </div>
      )}
