...
export function calcularFrete({
  ...
  if (isGuindauto) {
    ...
    // Guindauto é sempre externo — sem desconto Mills
    valorIda     = base
    valorRetorno = Math.round(base * 0.30 * 100) / 100
    const total  = Math.round((valorIda + valorRetorno) * 100) / 100
    return {
      km, veiculoId: vid,
      veiculoLabel: VEICULOS.find(v=>v.id===vid)?.label || vid,
      tipoFrete, subtype, isGuindauto: true,
      valorIda:         Math.round(valorIda * 100) / 100,
      valorRetorno,
      valorRetornoLabel:`${formatBRL(valorRetorno)} (30% — condição Mills × Hengel)`,
  ...
  // ── Frete Mills (Valdir — Bi-truck) ─────────────────────────
  if (tipoFrete === 'interno') {
    const baseIda = getValorIda(km, 'bitruck') * ajuste
    valorIda      = Math.round(baseIda * 0.70 * 100) / 100  // 30% abaixo da Hengel
