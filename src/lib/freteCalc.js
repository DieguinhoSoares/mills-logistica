...
// Diárias e Escolta: valores negociados conforme planilha 2026
...
// Baseado no peso operacional de cada família de equipamento (BID 2024)
const GRUPO_VEICULO = {
...
/**
 * Seleciona veículo pelo peso combinado das máquinas (Frete Externo multi-carga).
 */
 ...
 *   - Veículo: sugerido pelo grupo de modelo ou selecionado manualmente
 *   - Retorno: 30% da ida (condição comercial Mills × Hengel)
 *
 * Guindauto:
 *   - R$ 6,00/km fixo, retorno = ida (se retornar ao pátio)
 */
export function calcularFrete({
  ...
  const vid    = tipoFrete === 'interno' ? veiculoId : veiculoId
  if (!vid) return null
  ...
  // ── Frete Mills (interno) ────────────────────────────────────
  // Valdir (bitruck): 35% abaixo da Hengel — 1 máquina
  // Bento (prancha3): 30% abaixo da Hengel — 2+ máquinas
  // Ida e volta com mesmo valor (volta carregada)
  if (tipoFrete === 'interno') {
    if (vid === 'prancha3') {
      const baseIda = getValorIda(km, 'prancha3') * ajuste
      valorIda     = Math.round(baseIda * 0.70 * 100) / 100  // 30% abaixo Hengel
    } else {
      const baseIda = getValorIda(km, 'bitruck') * ajuste
      valorIda     = Math.round(baseIda * 0.65 * 100) / 100  // 35% abaixo Hengel
    }
