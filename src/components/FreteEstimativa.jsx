...
export function FreteEstimativa({ request, simClients = [], readOnly = false }) {
  ...
  // ── Sugestão / seleção de veículo ───────────────────────────
  useEffect(() => {
    // Guindauto: veículo fixo
    if (isGuindauto) {
    ...
    if (tipoFrete === 'interno') {
      // FIX: conta TOTAL de máquinas = reserva (vai) + danificada (retorna)
      // nInternosReserva → máquinas novas enviadas ao cliente
      // nInternos        → máquinas do pedido (inclui danificadas no sinistro)
      const nReservas    = request?.nInternosReserva?.length || 0
      const nDanificadas = request?.nInternos?.length || 0
      const totalMaquinas = Math.max(nReservas + nDanificadas, 1)

      // 1 máquina → Valdir (bi-truck) | 2+ máquinas → Bento (prancha3)
      const vid       = totalMaquinas > 1 ? 'prancha3' : 'bitruck'
      const motorista = totalMaquinas > 1
        ? `Prancha 3 eixos (Bento) · ${totalMaquinas} máquinas`
        : 'Bi-truck (Valdir)'
  ...
  useEffect(() => {
    ...
    // FIX: para frete interno, usa o veiculoId state (bitruck ou prancha3)
    // que foi determinado corretamente pelo número total de máquinas.
    // Não hardcoda mais 'bitruck' — respeita a seleção do useEffect acima.
    const vid = tipoFrete === 'interno'
      ? (veiculoId || 'bitruck')                        // fallback defensivo
      : (veiculoId || (readOnly ? 'prancha3' : ''))
  ...
  return (
    <div style={{ background:T.surfaceAlt, borderRadius:T.rLg, padding:16, border:`1px solid ${T.border}`, marginTop:12, marginBottom:12 }}>
      ...
      {/* Veículo Mills interno — informativo */}
      {isInterno && grupoLabel && (
        <div style={{ marginBottom:10, padding:'6px 10px', background:T.verdeLight, borderRadius:T.rSm, color:T.verde, fontSize:10, fontFamily:FONT, fontWeight:700 }}>
          🏢 Frete Mills · {grupoLabel}
        </div>
      )}
      ...
      {!readOnly && (
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12 }}>
          ...
          {/* Tipo de frete — só exibe quando não está determinado pelo tipo de solicitação */}
          {!isGuindauto && request?.type !== 'freteMillsInterno' && (
          <div>
