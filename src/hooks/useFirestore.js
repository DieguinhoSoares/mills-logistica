...
export function useSimClients() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db,'config','simClients'), snap=>{ if(snap.exists()) setSimClients(snap.data().clients||[]) }, ()=>{})
    return unsub
  }, [])
  const uploadClients = async clients => setDoc(doc(db,'config','simClients'), { clients, updatedAt:serverTimestamp() })
