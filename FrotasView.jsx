...
function CardModal({ card, defaultDate, simClients, onSave, onClose, onDelete }) {
  ...
  const handleSave = async () => {
    ...
    catch (err) { console.error('Erro ao salvar serviço:', err); alert('Erro ao salvar. Tente novamente.') }
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(26,22,18,.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(4px)' }}
      ...
      <motion.div initial={{ scale:.95, opacity:0, y:10 }} animate={{ scale:1, opacity:1, y:0 }}
        ...
        {/* Foto e nota de conclusão — visível apenas quando card foi concluído pelo motorista */}
        {(card?.conclusaoFoto || card?.conclusaoNota) && (
          <div style={{ marginTop:16, padding:'12px 14px', background:T.verdeLight, borderRadius:T.r, border:`1px solid ${T.verde}30` }}>
            <div style={{ color:T.verde, fontFamily:FONT, fontWeight:700, fontSize:11, marginBottom:8, textTransform:'uppercase', letterSpacing:'0.06em' }}>
              ✅ Registro de Conclusão pelo Motorista
            </div>
            {card.conclusaoNota && (
              <div style={{ color:T.textSec, fontFamily:FONT, fontSize:12, fontStyle:'italic', marginBottom:8, padding:'6px 10px', background:'rgba(255,255,255,0.6)', borderRadius:T.rSm }}>
                "{card.conclusaoNota}"
              </div>
            )}
            {card.conclusaoFoto && (
              <img src={card.conclusaoFoto} alt="Foto de conclusão"
                style={{ width:'100%', borderRadius:T.r, maxHeight:220, objectFit:'cover', cursor:'pointer' }}
                onClick={()=>window.open(card.conclusaoFoto,'_blank')}
              />
            )}
          </div>
        )}
...
// ── AssignDriverModal ─────────────────────────────────────────────────────────
function AssignDriverModal({ req, drivers, simClients, cards, onConfirm, onCancel }) {
      ...
      if (card.requestId) {
        ...
        if (card.requesterId) {
          await addDoc(collection(db,'notifications'), {
            ...
            message:`O serviço de ${card.machine||card.client||'sua solicitação'} foi concluído com sucesso.`,
